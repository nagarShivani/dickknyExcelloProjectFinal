...
export class BrandAddEditComponent {
  ...
  constructor(
    ...
    this.activaedroute.params.subscribe((params: any) => {
      ...
      if (this.paramsid) {
        this.loading = true;
        this.ErpService.getAllBrandById(this.paramsid).subscribe(
          (res: any) => {
            this.title = res?.name;
            this.loading = false;
          },
  ...
  SubmitEvent() {
    ...
    if (this.paramsid) {
      this.ErpService.updateBrand(this.paramsid, payload).subscribe(
        (res: any) => {
          this.ErpService.toast.snackbarError("Brand Updated Successfully");

          this.router.navigate(["/main/brand"]);
        }
      );
    } else {
      this.ErpService.addBrand({ body: payload }).subscribe((res: any) => {
        this.ErpService.toast.snackbarError("Brand Added Successfully");
